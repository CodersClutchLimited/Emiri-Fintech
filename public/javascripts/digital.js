...
// pause animation on
document.addEventListener("DOMContentLoaded", () => {
    const slider = document.getElementById("digital-auto-slider-2");
  ...
  document.addEventListener("DOMContentLoaded", () => {
    const slider = document.getElementById("digital-auto-slider");
    const sliderChildren = [...slider.children];
    ...
    function scrollSlider() {
      scrollAmount += 1; // Increment scrollAmount for left-to-right movement
      if (scrollAmount >= sliderChildren[0].offsetWidth) {
        scrollAmount = 0; // Reset scroll amount when a full slide has passed
        slider.prepend(slider.lastElementChild); // Move last element to the beginning
      }
      slider.style.transform = `translateX(${scrollAmount}px)`; // Apply transformation
      requestAnimationFrame(scrollSlider);
    }
